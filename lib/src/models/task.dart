...
class Task<Model> {
  ...
  void _register() {
    if (!_stateController.isClosed) {
      ...
      _subscriptionToFuture = _future.asStream().listen((event) {
        if (!_stateController.isClosed) {
          _stateController
              .add(TaskResult<Model>(status: TaskStatus.Success, value: event));
        }
      })
        ..onError((e) {
          ...
          if (!_stateController.isClosed) {
            _stateController.add(TaskResult<Model?>(
                status: TaskStatus.Error, errorTrace: trace));
          }
        });
    }
  }
  ...
  @override
  bool operator ==(other) {
    return other is Task<Model> &&
        hashCode == other.hashCode &&
        other._creationDate.isAtSameMomentAs(_creationDate);
  }
