...
void main() {
  group('Unit tests', () {
    test(
        'Manager is updated twice: 1st when the task added to the table, the second time: value is updated',
        () async {
    ...
    test('Any', () async {
      ...
      var updateCounter = 0;
      final counterManager = CounterManager();
      bool shouldUpdateListener(TaskStatus old, TaskStatus newStatus) =>
          newStatus != old;
      void listenerUpdate() {
        if (shouldUpdateListener(status,
            counterManager.statusTable['increment'] ?? TaskStatus.None)) {
          updateCounter++;
        }
      }
